...
def get_available_nodes():
    """Returns a set of node class names available in the current Nuke environment.
 
    The function builds the result from multiple sources when running inside Nuke:
    ``nuke.nodeTypes()``, menu items that contain ``createNode`` calls, and
    plugin files visible to the application. A small fallback list is used when
    ...
    Returns:
        set[str]: Set of node class names available in the current environment.
    """
